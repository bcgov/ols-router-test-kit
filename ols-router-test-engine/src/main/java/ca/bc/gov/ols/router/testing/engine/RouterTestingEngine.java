...
public class RouterTestingEngine {
	...
	@Autowired
    public RouterTestingEngine(ResultRepository resultRepository, DatasetRepository datasetRepository, EnvironmentRepository environmentRepository, RunRepository runRepository, TestRepository testRepository) {
		//initialize all our database access repositories so we can easily query anything with a single static variable per table.
	...
	public static void main(String[] args) {
		...
		for(Run run: runList) {
			...
			try {
				...
				List<Result> results = runTests(saveResults, list, run);
	...
	private static List<Result> runTests(Boolean saveResults, List<Test> list, Run run) throws IOException {
	...
	private static Result runSingleTest(Map<String, String> envParameters, Map<String, String> parameters, Integer runId, Integer testId ) throws IOException {
		String baseUrl = "";
		if("true".equals(parameters.get("followTruckRoute"))) {
			baseUrl = envParameters.get("truckurl");	
		}else {//sometimes this param is not listed at all, but we assume 'false' in that case as well with this 'else' logic
		...
		//System.out.println("testing URL: " + ParameterStringBuilder.getParamsString(parameters));
		...
		if("isTruckRoute".equals(partition)) {
			...
			partitionIndices = StringUtils.chop(partitionIndices); //get rid of the trailing separator, '|'  once the string is complete.  
		}
		
		ArrayList pointList = (ArrayList)attributes.get("route");
		Coordinate[] pointArray = new Coordinate[pointList.size()];

		//convert to Coordinate[] for Geometry type construction, can't seem to cast to the correct object type just using .toArray(), so we do this instead: 
		...
		Result result = new Result(runId, testId,(double)attributes.get("executionTime"),(double)attributes.get("distance"),route,(double)attributes.get("time"),partitionSignature, partitionIndices);
